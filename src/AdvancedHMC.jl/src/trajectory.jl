...
$(TYPEDFIELDS)
"""
struct Transition{P<: AbstractPhasePoint, NT<:NamedTuple}
...
$(TYPEDFIELDS)
"""
struct SliceTS{P<:AbstractPhasePoint, F<:AbstractFloat} <: AbstractTrajectorySampler
...
"""
SliceTS(rng::AbstractRNG, z0::P) where {P<:AbstractPhasePoint} = SliceTS(z0, log(rand(rng)) - energy(z0), 1)
...
"""
function SliceTS(s::SliceTS, H0::AbstractFloat, zcand::P) where {P<:AbstractPhasePoint}
...
function combine(zcand::P, s1::SliceTS, s2::SliceTS) where {P<:AbstractPhasePoint}
    @assert s1.ℓu == s2.ℓu "Cannot combine two slice sampler with different slice variable"
    n = s1.n + s2.n
    return SliceTS(zcand, s1.ℓu, n)
end
...
GeneralisedNoUTurn(z::P) where {P <: AbstractPhasePoint} = GeneralisedNoUTurn(z.r)
...
"""
function isterminated(h::H, t::BinaryTree{<:ClassicNoUTurn}) where {H <: AbstractHamiltonian}
...
"""
function isterminated(h::H, t::BinaryTree{<:GeneralisedNoUTurn}) where {H <: AbstractHamiltonian}
    rho = t.c.rho
...
function isterminated(
    h::H, t::T, tleft::T, tright::T
) where {H<:AbstractHamiltonian, T<:BinaryTree{<:StrictGeneralisedNoUTurn}}
...
function isterminated(h::H, t::BinaryTree{T}, args...) where {H<:AbstractHamiltonian, T<:Union{ClassicNoUTurn, GeneralisedNoUTurn}}
